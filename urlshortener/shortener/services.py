...
def create_or_get_short_url(*, long_url: str) -> Tuple[Shortner, bool]:
    existing_short_url = Shortner.objects.filter(long_url=long_url).first()
    ...
    shortener = Shortner(snowflake_id=snowflake_id, short_url=short_url, long_url=long_url)
    shortener.save()
